...
//-- 分頁判斷數 --
 ...
 $page=empty($_GET['page'])? 0:((int)$_GET['page']-1)*$num;
   ...
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT Tb_index, aTitle, StartDate, EndDate, OrderBy, OnLineOrNot, aPic FROM appProduct WHERE mt_id=:mt_id AND webLang=:webLang  ORDER BY OrderBy DESC, StartDate DESC LIMIT ".$page.",".$num);
