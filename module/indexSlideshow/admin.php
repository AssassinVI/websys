...
if ($_POST) {
  ...
  for ($i=0; $i <count($_POST['OrderBy']) ; $i++) { 
    ...
    pdo_update('indexSlideshow', $data, $where);
  }
}
   ...
   /*	$del_row=pdo_select('SELECT aPic, OtherFile FROM appArticle WHERE Tb_index=:Tb_index', $where);
    ...
    if (isset($del_row['OtherFile'])) {
      ...
      for ($i=0; $i <count($OtherFile)-1 ; $i++) { 
   	 ...
   	 pdo_delete('indexSlideshow', $where);
   }
   
   $pdo=pdo_conn();
   $sql=$pdo->prepare("SELECT Tb_index, aTitle, StartDate, OrderBy FROM indexSlideshow WHERE webLang=:webLang ORDER BY OrderBy DESC, StartDate DESC");
